...
// private(i, j)
    // 根据 P1~P7 组合求出目标子矩阵 C11, C12, C21, C22
    for (size_t i = 0; i < half; ++i) {
        const float* p1 = P1 + i*half;
        const float* p2 = P2 + i*half;
        const float* p3 = P3 + i*half;
        const float* p4 = P4 + i*half;
        const float* p5 = P5 + i*half;
        const float* p6 = P6 + i*half;
        const float* p7 = P7 + i*half;
...
#pragma omp parallel for  default(none) shared(N, A, B, C,BLOCK)
    for (size_t i_b = 0; i_b < N; i_b += BLOCK)
    {
        for (size_t j_b = 0; j_b < N; j_b += BLOCK)
        {
...
#pragma omp parallel for  default(none) shared(A, B, C, N,BLOCK)
    for (size_t i_b = 0; i_b < N; i_b += BLOCK)
    {
        for (size_t j_b = 0; j_b < N; j_b += BLOCK)
        {
...
void read_block_from_disk(FILE *f, Matrix const *block, size_t N, size_t row_start, size_t col_start)
{
    ...
    for (size_t i = 0; i < B; i++)
    {
        ...
        FILE_SEEK(f, offset, SEEK_SET); // Windows 下支持超大文件偏移的 fseek
        size_t items_read = fread(&block->data[i * B], sizeof(float), B, f);
        if (items_read != B) {
            // 输出错误信息或返回错误码
            fprintf(stderr, "Error reading block from disk at row %zu\n", row_start + i);
            // 可以选择清理并退出，或返回错误标志
        }
    }
}
